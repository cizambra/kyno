...
- Before adding a test, check what behavior it proves. Put isolated SDK and
  adapter tests in `surface/unit`, adapter tests against real framework APIs
  without a server in `surface/integration`, adapter/server round trips in
  `surface/e2e`, isolated Kyno module tests in `core/unit`, tests using
  multiple Kyno modules or a real store in `core/integration`, complete
  control-plane journeys in `core/e2e`, and repository checks in `checks`.
