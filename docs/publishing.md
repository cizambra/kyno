...
If your custom constitution template uses `$name` or `${name}`, replace it
with `$constitution_key` or `${constitution_key}`. Unsupported placeholders
remain literal text, so an unchanged template will display `$name` rather
than the constitution's key. Keep a copy of the template that matches your
deployed version if you need to roll back.
