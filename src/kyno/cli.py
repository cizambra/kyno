...
def _fetch_remote_head(client, constitution: str | None) -> dict:
    ...
    row = client.call_tool("get_constitution", arguments)
    _response_constitution_key(row, constitution)
    return row


def _response_constitution_key(row: dict, expected: str | None) -> str:
    key = check_constitution_key(row.get("constitution_key"))
    if expected is not None and key != expected:
        raise CoherenceError(
            f"response constitution key '{key}' does not match requested key '{expected}'"
        )
    return key
...
def _fetch_remote_rows(
    ...
    *,
    ...
    client = dial(profile, credentials_profile=credentials, token_env=token_env)
    try:
        rows = client.call_tool("export_versions", arguments)
        expected = constitution
        for row in rows:
            expected = _response_constitution_key(row, expected)
        return rows
    finally:
        client.close()
