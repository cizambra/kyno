...
def test_only_marker_first_text_counts_as_a_block():
...
def test_refresh_puts_the_fresh_block_first_and_removes_the_old_one():
    texts = [BLOCK_V1, "role prompt", "task"]
    assert refresh(texts, BLOCK_V2) == [BLOCK_V2, "role prompt", "task"]


def test_refresh_on_a_list_without_a_block_just_prepends():
    assert refresh(["role prompt"], BLOCK_V1) == [BLOCK_V1, "role prompt"]


def test_refresh_removes_every_stale_block_however_many():
...
def test_a_block_decision_halts_any_host():
    assert decision(Action.BLOCK).halts(can_pause=True)
    assert decision(Action.BLOCK).halts(can_pause=False)


def test_a_pause_decision_halts_only_a_host_that_cannot_pause():
