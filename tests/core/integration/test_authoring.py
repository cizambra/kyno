...
@pytest.mark.parametrize("selected_key", [None, "support", "sales"])
@pytest.mark.parametrize("command", ["check", "apply"])
def test_given_constitution_field_when_cli_check_or_apply_then_error_prevents_writes(
    db, tmp_path, selected_key, command
):
    document = {"constitution": "support", "mission": "Help customers"}
    if selected_key is not None:
        document["constitution_key"] = selected_key
    path = tmp_path / "direction.json"
    path.write_text(json.dumps(document))
    arguments = [command, str(path)]
    if command == "apply":
        arguments.extend(["--note", "Initial direction"])

    result = runner.invoke(app, arguments)

    assert result.exit_code != 0
    assert "constitution_key" in plain(result)
    assert "'constitution' is unsupported" in plain(result)
    control_plane = plane(db)
    assert control_plane.current().version == 0
    assert control_plane.current("support").version == 0
