...
@pytest.mark.parametrize(
    ...
    ids=["current", "current---yaml", "get-version-1", "get-version-1---yaml", "history", "export"],
)
def test_given_mismatched_key_when_cli_read_command_runs_then_error_prevents_output(
    ...
    remote.call_tool.return_value = (
        version.to_dict() if command[0] == "current" else [version.to_dict()]
    )
    monkeypatch.setattr(cli, "dial", lambda *args, **kwargs: remote)

    result = runner.invoke(cli.app, [*command, "--remote", "--constitution-key", " support "])
...
@pytest.mark.parametrize("command", ["history", "export"])
@pytest.mark.parametrize("explicit", [False, True])
def test_given_mixed_keys_when_cli_history_or_export_then_error_prevents_output(
    monkeypatch, memory_store, command, explicit
):
    plane = ControlPlane(memory_store)
...
def test_given_no_cli_key_when_cli_command_runs_then_core_receives_none(
...
@pytest.mark.parametrize(
    "command",
    [["current"], ["get-version", "1"], ["history"], ["export"]],
    ids=["current", "get-version-1", "history", "export"],
)
def test_given_no_remote_cli_key_when_cli_read_command_runs_then_mcp_arguments_omit_the_key(
    monkeypatch, memory_store, command
):
    plane = ControlPlane(memory_store)
...
@pytest.mark.parametrize(
    "command",
    [["current"], ["current", "--yaml"], ["get-version", "1"], ["export"]],
    ids=["current", "current---yaml", "get-version-1", "export"],
)
@pytest.mark.parametrize(
    "identity",
    [
...
def test_given_invalid_remote_identity_when_cli_read_command_runs_then_error_is_clean(
...
@pytest.mark.parametrize(
    "command",
    [["current", "--yaml"], ["get-version", "1", "--yaml"], ["export"]],
    ids=["current---yaml", "get-version-1---yaml", "export"],
)
def test_given_server_selected_key_when_cli_command_omits_key_then_output_names_returned_key(
    monkeypatch, memory_store, command
):
    plane = ControlPlane(memory_store)
    version = plane.apply_direction(
...
def test_given_core_publication_identity_when_cli_command_omits_key_then_output_names_returned_key(
    monkeypatch, command
):
    plane = Mock()
