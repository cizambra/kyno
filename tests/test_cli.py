...
def test_given_the_retired_range_flags_when_exporting_then_they_are_refused(tmp_path, monkeypatch):
    # Export writes what import reads: a whole ledger. Reading a slice of
    # history is jq's job, or kyno log's.
    cli_workspace(monkeypatch, tmp_path, tmp_path / "c.sqlite3")
    runner.invoke(app, ["db", "init"])

    r = runner.invoke(app, ["export", "--from", "2"])
