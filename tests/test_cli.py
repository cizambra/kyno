...
def test_given_an_empty_store_when_applying_then_the_first_version_is_announced(tmp_path, monkeypatch):
    monkeypatch.setenv("KYNO_DATABASE_URL", f"sqlite:///{tmp_path / 'c.sqlite3'}")
    runner.invoke(app, ["init-db"])
    r = apply_yaml(tmp_path, mission="M1", note="init")
...
def test_given_identical_content_when_dry_running_then_it_says_no_field_changed(tmp_path, monkeypatch):
    monkeypatch.setenv("KYNO_DATABASE_URL", f"sqlite:///{tmp_path / 'c.sqlite3'}")
    runner.invoke(app, ["init-db"])
